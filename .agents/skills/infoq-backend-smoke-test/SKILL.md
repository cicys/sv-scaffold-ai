...
- 在隔离端口启动后端，并关闭验证码。
- 默认优先使用 `application-local.yml` 对应的 `local` profile；若文件不存在，则回退到 `application-dev.yml` 与 `dev` profile。
...
```bash
node .agents/skills/infoq-backend-smoke-test/scripts/run_smoke.mjs
```

双节点 WebSocket 集群 smoke：

```bash
node .agents/skills/infoq-backend-smoke-test/scripts/run_cluster_smoke.mjs
```
...
node .agents/skills/infoq-backend-smoke-test/scripts/run_smoke.mjs --build
...
node .agents/skills/infoq-backend-smoke-test/scripts/run_smoke.mjs --keep-server

# Build first, then run two-node WebSocket cluster smoke
node .agents/skills/infoq-backend-smoke-test/scripts/run_cluster_smoke.mjs --build
...
- 双节点 cluster smoke 默认值：
  ...
  - Redis：默认按 `application-local.yml -> application-dev.yml` 顺序解析当前 Redis 配置，可通过 `--redis-*` 或 `CLUSTER_SMOKE_REDIS_*` 覆盖
...
- 主入口：`scripts/run_smoke.mjs`
- 双节点 smoke：`scripts/run_cluster_smoke.mjs`
