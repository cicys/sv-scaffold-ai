...
1. 若当前环境支持 `bash`，可使用 `scripts/start_admin_dev_stack.sh` 启动或重启 backend + React admin 栈。
2. 若当前环境不适合执行 `bash`，手动启动 backend 与 React admin；启动前先确认 `java -version` 与 `mvn -version` 指向 JDK 17。
...
- 当前环境若无法执行 `bash`，应手动启动 backend / React admin 后继续使用跨平台 `playwright-cli`；不要因为包装脚本不可用而跳过运行态验证。
