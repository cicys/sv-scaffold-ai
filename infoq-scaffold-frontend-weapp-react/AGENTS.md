...
|Local Skills:.codex/skills:{infoq-react-unit-test-patterns,infoq-react-runtime-verification}
...
|E2E Determinism:`test:e2e:weapp:core` 默认无后端稳定模式（`WEAPP_E2E_AUTO_LOGIN=false` 且不注入 mock token）。|真实登录联调仅使用 `test:e2e:weapp:core:backend`，并要求后端可用且 `--captcha.enable=false`。|运行态验证优先使用 infoq-react-runtime-verification 的 `run_weapp_smoke.mjs`。
