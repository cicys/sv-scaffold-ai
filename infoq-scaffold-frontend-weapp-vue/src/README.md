...
`src/` 是 weapp Vue 的主实现层，承载应用入口、页面、会话状态、页面级鉴权、请求封装和运行时工具。
...
| `composables/components/utils` | 承载跨页面鉴权、UI 与运行时辅助能力 |

## 4. 依赖方向

- `pages` 依赖 `composables`、`store`、`api`、`components`、`utils`。
- `api` 统一依赖 `api/request.ts`。
- `store/session.ts` 依赖 `api/index.ts` 聚合导出的登录、用户和权限工具。
- `composables/use-auth-guard.ts` 依赖 `store/session.ts` 与 `utils/navigation.ts`。
...
- 当前没有中心化路由守卫目录；页面访问控制通过 composable 和页面自身组合完成。
